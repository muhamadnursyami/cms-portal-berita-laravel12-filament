...
@section('title', 'Website Portal Berita')
...
@section('content')
    ...
    <div class="flex flex-col px-14 mt-10 ">
        ...
        <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-4">
            @foreach ($features as $feature)
                <a href="{{ route('news.show', $feature->slug) }}">
                    <div
                        class="border height-full border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer transition duration-300 ease-in-out">
                        <div
                            class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal ml-2 mt-2 text-sm absolute">
                            {{ $feature->newsCategory->title }}</div>
                        <img src="{{ asset('storage/' . $feature->thumbnail) }}" alt=""
                            class="w-full rounded-xl mb-3 h-[150px] object-cover">
                        <p class="font-bold text-base mb-1">{{ $feature->title }}</p>
                        <p class="text-slate-400">{{ \Carbon\Carbon::parse($feature->created_at)->format('d F Y') }} </p>
                    </div>
                </a>
            @endforeach
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div
                ...
                <a href="{{ route('news.show', $news[0]->slug) }}">
                    ...
                    <p class="text-slate-400 text-base mt-1">
                        {{ \Carbon\Carbon::parse($news[0]->created_at)->format('d F Y') }}</p>
                </a>
            </div>
            ...
            @foreach ($news->skip(1) as $new)
                ...
                <a href="{{ route('news.show', $new->slug) }}"
                    ...
                    <img src="{{ asset('storage/' . $new->thumbnail) }}" alt="berita2"
                        class="rounded-xl w-[80%] md:max-h-48 object-cover">
    ...
    <div class="flex flex-col px-14 mt-10 mb-10">
        ...
        <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-4">
            @foreach ($news as $pilihanAuthor)
                <a href="{{ route('news.show', $pilihanAuthor->slug) }}">
                    <div
                        ...
                        <div
                            class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal ml-2 mt-2 text-sm absolute">
                            {{ $pilihanAuthor->newsCategory->title }}</div>
                        <img src="{{ asset('storage/' . $pilihanAuthor->thumbnail) }}" alt=""
                            class="w-full rounded-xl mb-3">
                        <p class="font-bold text-base mb-1"> {{ $pilihanAuthor->title }}</p>
                        <p class="text-slate-400"> {{ \Carbon\Carbon::parse($pilihanAuthor->created_at)->format('d F Y') }}
                        </p>
                    </div>
                </a>
            @endforeach
